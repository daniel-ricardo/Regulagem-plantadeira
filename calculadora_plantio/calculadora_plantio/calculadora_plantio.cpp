...
{
    frameMain::regulagem->setCult("teste");
    frameMain::regulagem->setFormAdb("00-00-00");
    frameMain::regulagem->setVariSmt("original");
    frameMain::regulagem->setEspc(static_cast<float>(this->inEspc->GetValue()));
    frameMain::regulagem->setTiro(static_cast<float>(this->inTiro->GetValue()));
    frameMain::regulagem->setGramA(calcularGramasTiro(static_cast<float>(this->inKgAdb->GetValue()), frameMain::regulagem));
    frameMain::regulagem->setGramS(calcularGramasTiro(static_cast<float>(this->inKgSmt->GetValue()), frameMain::regulagem));

    this->outAdb->SetValue( std::to_string(frameMain::regulagem->getGramAdb()) );
    this->outSmt->SetValue( std::to_string(frameMain::regulagem->getGramSmt()) );
}
    ...
    const wxPoint& pos, const wxSize& size, long style, const wxString& name) : wxFrame()
{
    ...
    // Preenche valores de output iniciais

    this->outAdb->SetValue( std::to_string(std::ceil(regulagem->getGramAdb())) );
    this->outSmt->SetValue( std::to_string(std::ceil(regulagem->getGramSmt())) );
}
